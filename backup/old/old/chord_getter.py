...
def extract_chords_and_lyrics(html: str, url: str):
    ...
    if len(h1_tags) >= 2:
        # take the second h1, then the text of the <a> inside
        a_tag = h1_tags[1].find("a")
        title = a_tag.get_text(strip=True) if a_tag else h1_tags[1].get_text(strip=True)
    else:
        title = "Unknown title"

    # Find raw artist
    a_tag = soup.find('h2', class_='pull-right').find('a')
    raw_artist = a_tag.get_text()


    # artist, year = get_song_info(title, raw_artist)
    artist = raw_artist
    year = 1999  
    ...
    return {
        ...
        "artist": raw_artist,
        "year": year,
        "chords_and_lyrics": aligned_text
    }
...
def extract_multiple(urls: List[str]) -> List[Dict[str, str]]:
    results = []
    for url in urls:
        print(f"Extracting: {url}")
        song = extract_song_aligned(url)
        if song:
            print(f" → {song['title']}")
            results.append(song)
        else:
...
def main():
    with open('song_url_list.json') as f:
        songs = json.load(f)
    song_urls = songs.values()
    songs = get_contents(song_urls)
    ...
    for s in songs:
        ...
        print(f"Group: {s['artist']}")
        print(f"Year: {s['year']}")
        print(f"URL: {s['url']}")
        print("Chords & Lyrics:\n")
        print(s['chords_and_lyrics'])
        print("="*40 + "\n")
